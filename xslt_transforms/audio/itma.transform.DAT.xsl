<xsl:stylesheet version="1.0" 
	...
	<xsl:template name="Genre">
		<mods:genre authority="aat" type="Concept" displayLabel="Format" authorityURI="http://vocab.getty.edu/aat/" valueURI="http://vocab.getty.edu/aat/300265821">digital audio tapes</mods:genre>
	</xsl:template>
	...
	<xsl:template name="getRelatedItems">			
		<xsl:for-each select="*[starts-with(name(),'Track')]">
			<xsl:variable name="disc_number" select="position()"/>
			<xsl:variable name="disc_content" select="."/>
			...
			<xsl:for-each select="exsl:node-set($tracks)/mods:titleInfo/mods:title">
				<mods:relatedItem type="constituent">
					<xsl:attribute name="ID">DMD_part0<xsl:value-of select="$disc_number"/>_track0<xsl:value-of select="position()"/></xsl:attribute>
					<mods:titleInfo>
						<xsl:copy-of select="."/> 
					</mods:titleInfo>
				</mods:relatedItem>
